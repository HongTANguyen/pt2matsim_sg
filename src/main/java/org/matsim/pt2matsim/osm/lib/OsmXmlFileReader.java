 ...
 */
public class OsmXmlFileReader extends MatsimXmlParser {
	...
	public OsmXmlFileReader(OsmData osmData) {
	...
	@Override
	public void startTag(final String name, final Attributes attrs, final Stack<String> context) {
		if ("node".equals(name)) {
			...
			this.currentWay = new ParsedWay(Long.parseLong(attrs.getValue("id")));
		} else if ("relation".equals(name)) {
			String id = StringCache.get(attrs.getValue("id"));
			this.currentRelation = new ParsedRelation(Long.parseLong(id));
		} else if ("nd".equals(name)) {
			if (this.currentWay != null) {
				this.currentWay.nodes.add(Long.valueOf(attrs.getValue("ref")));
			}
		} else if ("tag".equals(name)) {
			if (this.currentNode != null) {
				this.currentNode.tags.put(StringCache.get(attrs.getValue("k")), StringCache.get(attrs.getValue("v")));
			} else if (this.currentWay != null) {
				this.currentWay.tags.put(StringCache.get(attrs.getValue("k")), StringCache.get(attrs.getValue("v")));
			} else if (this.currentRelation != null) {
				this.currentRelation.tags.put(StringCache.get(attrs.getValue("k")), StringCache.get(attrs.getValue("v")));
			}
		} else if ("member".equals(name)) {
			if (this.currentRelation != null) {
				Osm.ElementType type;
				String lcType = attrs.getValue("type").toLowerCase(Locale.ROOT);
                type = switch (lcType) {
                    case "node" -> Osm.ElementType.NODE;
                    case "way" -> Osm.ElementType.WAY;
                    case "relation" -> Osm.ElementType.RELATION;
                    default -> null;
                };
				this.currentRelation.members.add(new ParsedRelationMember(type, Long.parseLong(attrs.getValue("ref")), StringCache.get(attrs.getValue("role"))));
			}
		}
	}
	...
	public record ParsedRelationMember(Osm.ElementType type, long refId, String role) {
	}
}